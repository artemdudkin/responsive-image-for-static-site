...
function processImg(imgObj, cacheFolder) {
    let ret = undefined;
    const src = (imgObj.attrs.filter(_=>_.name==='pi-original')[0] || imgObj.attrs.filter(_=>_.name==='src')[0] || {}).value;
    ...
    if (srcSize.height > 100) {
      ret = sharp(src)
        ...
        .toBuffer()
        .then( srcStub => {
          addOrReplace( imgObj.attrs, "pi-stub", 'data:image/png;base64,' + srcStub.toString('base64'));
          addOrReplace( imgObj.attrs, "pi-original", src);
